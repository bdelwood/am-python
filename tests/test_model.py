...
ARGS = ["0", "GHz", "350", "GHz", "0.01", "GHz", "35", "deg", "1.0"]
...
def test_sequential_runs():
    ...
    for i in range(3):
        m = am.Model(AMC, ARGS)
        m.compute()
        assert m.frequency.shape == (35001,), f"run {i}: wrong grid size"
        assert m.transmittance is not None, f"run {i}: no transmittance"
        np.testing.assert_allclose(m.transmittance[15000], 0.9707, atol=1e-3)
        del m
